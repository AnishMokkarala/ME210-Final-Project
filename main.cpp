...
/*---------------State Definitions--------------------------*/
typedef enum {
    WAITING, LOADING, MOVE, TURN, ALIGN, SCORING, OFF
} Robot_states;
...
void checkGlobalEvents(void){
  ...
  if (digitalRead(LIM_LOW)==HIGH && robot_state!= SCORING && robot_state!= OFF){ //If bottom limit is unpressed and not scoring
    digitalWrite(EN_STEP,LOW);
    myStepper1.runSpeed();
  }
  else if (robot_state !=SCORING) digitalWrite(EN_STEP,HIGH);
...
void jiggle(void){
  delay(200);
  motorsStraight(HIGH,HIGH,255);
  delay(200);
  motorsStraight(LOW,LOW,200);
}